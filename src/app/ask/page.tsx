...
export default function Home() {
  ...
  async function ask(raw: string) {
    ...
    try {
      ...
      if (epoch === turnEpochRef.current) {
        setTurns((t) => [
          ...t,
          { who: personaTitle(personaId), text: answer, sources: answerSources },
        ]);
      }
    } catch (err) {
  ...
  const persona = personaTitle(personaId);

  return (
    // h-full: the notch shell's viewport owns the height, not the raw viewport
    <main className="relative flex h-full min-h-[480px] flex-col overflow-x-hidden overflow-y-auto bg-white text-[#0A0A0A] font-[family-name:var(--font-grotesk)]">
      ...
      <div
        className="fixed bottom-0 right-0 top-0 z-[60] flex w-[min(400px,88vw)] flex-col border-l border-[#EAEAEA] bg-white transition-transform duration-500"
        style={{
